...
class HomogeneousIsotropicTurbulenceGenerator:
    """
        ...
        поля однородной изотропоной турбулентности и сохранения данных о
    ...
    @classmethod
    def _get_index_shift(cls, parameter_arr: typing.List[float], number: int, num: int, delta_i: int = 1,
    ...
    def run(self, **kwargs):
        ...
        index_gen3 = self._get_index_generator(self.num)
        ...
        self._create_plot3d_file(self.plot3d_filename, self.num, self._x_arr, self._y_arr,
                                 self._z_arr)
...
if __name__ == '__main__':
    turb_generator = HomogeneousIsotropicTurbulenceGenerator(3, 'output\data_files', 0.001, 0.005,
                                                             2e-5, 6e3)
