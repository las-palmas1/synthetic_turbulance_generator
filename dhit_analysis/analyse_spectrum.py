...
def make_plot_with_exp(frames_set1: typing.List[pd.DataFrame], name1: str, num1: int,
                       ...
                       save_name='spectrum_history_with_exp_data.png'):
    """
...
def make_kinetic_energy_plot(frames_set1: typing.List[pd.DataFrame], name1: str, num1: int, sol_time_arr1,
                             ...
                             ylim=(0, 0.05), scale='linear', save_name='kinetic_energy.png'):
    """
...
if __name__ == '__main__':
    lazurit_frames1, lazurit_sol_time_arr1 = get_frames_set_and_sol_time(os.path.join(config.lazurit_data_dir, '1e-4'))
    lazurit_frames2, lazurit_sol_time_arr2 = get_frames_set_and_sol_time(os.path.join(config.lazurit_data_dir, '5e-5'))
    ...
    make_comparison_plot(lazurit_frames1, 'Lazurit, time step = 1e-4', config.num + 1,
                         lazurit_frames2, 'Lazurit, time step = 5e-5', config.num + 1, lazurit_sol_time_arr1,
                         save_name='spectrum_history_lazurit_64cells.png')

    # ---------------------------------------------------------------------------
    # Создание графика истории изменения спектра в ходе расчета и эксперимента
    # ---------------------------------------------------------------------------
    make_plot_with_exp(lazurit_frames1, '1e-4', config.num+1, lazurit_frames2, '5e-5', config.num+1,
                       save_name='spectrum_history_with_exp_data_lazurit_64cells.png')
    # --------------------------------------------------------------------------------
    # созданиен графика изменения кинетической энергии турбулентности
    # --------------------------------------------------------------------------------
    make_kinetic_energy_plot(lazurit_frames1, '1e-4', config.num+1, lazurit_sol_time_arr1,
                             lazurit_frames2, '5e-5', config.num + 1, lazurit_sol_time_arr2,
                             save_name='kinetic_energy_lazurot_64cells.png')
    # --------------------------------------------------------------------------------
